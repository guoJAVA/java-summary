...
![image-20200728134155005](../../../media/pictures/java/multithread/image-20200728134155005.png)
...
下图是 Java 内存区域，通过下图我们从 JVM 的角度来说一下线程和进程之间的关系。如果你对 Java 内存区域 (运行时数据区) 这部分知识不太了解的话可以阅读一下这篇文章：[《可能是把 Java 内存区域讲的最清楚的一篇文章》](docs/Java/Java内存区域.md)

![JVM运行时数据区域](../../../media/pictures/java/multithread/JVM运行时数据区域.png)
...
> 原图中 wait到 runnable状态的转换中，`join`实际上是`Thread`类的方法，但这里写成了`Object`。
...
![2019-4死锁1](../../../media/pictures/java/multithread/2019-4死锁1.png)
