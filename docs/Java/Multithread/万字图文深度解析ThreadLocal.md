...
![1](../../../media/pictures/java/multithread/1.png)
...
![2](../../../media/pictures/java/multithread/2.png)
...
![3](../../../media/pictures/java/multithread/3.png)
...
![4](../../../media/pictures/java/multithread/4.png)
...
![8](../../../media/pictures/java/multithread/8.png)
...
![7](../../../media/pictures/java/multithread/7.png)
...
![9](../../../media/pictures/java/multithread/9.png)

这里直接将数据放到该槽位即可。

**第二种情况：** 槽位数据不为空，`key`值与当前`ThreadLocal`通过`hash`计算获取的`key`值一致：

![10](../../../media/pictures/java/multithread/10.png)

这里直接更新该槽位的数据。

**第三种情况：** 槽位数据不为空，往后遍历过程中，在找到`Entry`为`null`的槽位之前，没有遇到`key`过期的`Entry`：

![11](../../../media/pictures/java/multithread/11.png)
...
![12](../../../media/pictures/java/multithread/12.png)
...
![13](../../../media/pictures/java/multithread/13.png)
...
![14](../../../media/pictures/java/multithread/14.png)

从当前节点`staleSlot`向后查找`key`值相等的`Entry`元素，找到后更新`Entry`的值并交换`staleSlot`元素的位置(`staleSlot`位置为过期元素)，更新`Entry`数据，然后开始进行过期`Entry`的清理工作。

**向后遍历过程中，如果没有找到相同key值的Entry数据：**

![15](../../../media/pictures/java/multithread/15.png)
...
![16](../../../media/pictures/java/multithread/16.png)
...
![17](../../../media/pictures/java/multithread/17.png)
...
3. 1 如果`k = key`，说明当前`set`操作是一个替换操作，做替换逻辑，直接返回
4. 2 如果`key = null`，说明当前桶位置的`Entry`是过期数据，执行`replaceStaleEntry()`方法(核心方法)，然后返回
5. `for`循环执行完毕，继续往下执行说明向后迭代的过程中遇到了`entry`为`null`的情况
6. 1 在`Entry`为`null`的桶中创建一个新的`Entry`对象
7. 2 执行`++size`操作
8. 调用`cleanSomeSlots()`做一次启发式清理工作，清理散列数组中`Entry`的`key`过期的数据
9. 1 如果清理工作完成后，未清理到任何数据，且`size`超过了阈值(数组长度的2/3)，进行`rehash()`操作
10. 2 `rehash()`中会先进行一轮探测式清理，清理过期`key`，清理完成后如果**size >= threshold - threshold / 4**，就会执行真正的扩容逻辑(扩容逻辑往后看)
...
![18](../../../media/pictures/java/multithread/18.png)
...
![19](../../../media/pictures/java/multithread/19.png)
...
![24](../../../media/pictures/java/multithread/24.png)

接着看看具体的`resize()`方法，为了方便演示，我们以`oldTab.len=8`来举例：

![25](../../../media/pictures/java/multithread/25.png)
...
![26](../../../media/pictures/java/multithread/26.png)

**第二种情况：** `slot`位置中的`Entry.key`和要查找的`key`不一致：

![27](../../../media/pictures/java/multithread/27.png)

我们以`get(ThreadLocal1)`为例，通过`hash`计算后，正确的`slot`位置应该是4，而`index=4`的槽位已经有了数据，且`key`值不等于`ThreadLocal`，所以需要继续往后迭代查找。
...
![28](../../../media/pictures/java/multithread/28.png)
...
![29](../../../media/pictures/java/multithread/29.png)
...
![30](../../../media/pictures/java/multithread/30.png)

图中的`requestId`即为我们各个系统链路关联的`traceId`，系统间互相调用，通过这个`requestId`即可找到对应链路，这里还有会有一些其他场景：

![31](../../../media/pictures/java/multithread/31.png)
