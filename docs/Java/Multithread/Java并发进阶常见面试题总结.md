...
#### 1.2.说说自己是怎样使用synchronized关键字，在项目中用到了吗？
...
![synchronized关键字原理](../../../media/pictures/java/multithread/synchronized关键字原理.png)
...
![synchronized关键字原理2](../../../media/pictures/java/multithread/synchronized关键字原理2.png)

synchronized 修饰的方法并没有 monitorenter 指令和 monitorexit 指令，取得代之的确实是 ACC_SYNCHRONIZED 标识，该标识指明了该方法是一个同步方法，JVM 通过该 ACC_SYNCHRONIZED 访问标志来辨别一个方法是否声明为同步方法，从而执行相应的同步调用。

#### 1.4.说说JDK1.6之后的synchronized关键字底层做了哪些优化，可以详细介绍一下这些优化吗
...
自旋锁在 JDK1.6 之前其实就已经引入了，不过是默认关闭的，需要通过`--XX:+UseSpinning`参数来开启。JDK1.6及1.6之后，就改为默认开启的了。需要注意的是：自旋等待不能完全替代阻塞，因为它还是要占用处理器时间。如果锁被占用的时间短，那么效果当然就很好了！反之，相反！自旋等待的时间必须要有限度。如果自旋超过了限定次数仍然没有获得锁，就应该挂起线程。**自旋次数的默认值是10次，用户可以修改`--XX:PreBlockSpin`来更改**。
...
![数据不一致](../../../media/pictures/java/multithread/数据不一致.png)

要解决这个问题，就需要把变量声明为**volatile**，这就指示 JVM，这个变量是不稳定的，每次使用它都到主存中进行读取。

说白了， **volatile** 关键字的主要作用就是保证变量的可见性然后还有一个作用是防止指令重排序。

![volatile关键字的可见性](../../../media/pictures/java/multithread/volatile关键字的可见性.png)
...
![ThreadLocal内部类](../../../media/pictures/java/multithread/ThreadLocal内部类.png)
...
![Executor框架的工具类](../../../media/pictures/java/multithread/Executor框架的工具类.png)
...
![JUC原子类概览](../../../media/pictures/java/multithread/JUC原子类概览.png)
...
#### 5.4.能不能给我简单介绍一下AtomicInteger类的原理
...
![AQS类](../../../media/pictures/java/multithread/AQS类.png)

AQS是一个用来构建锁和同步器的框架，使用AQS能简单且高效地构造出应用广泛的大量的同步器，比如我们提到的ReentrantLock，Semaphore，其他的诸如ReentrantReadWriteLock，SynchronousQueue，FutureTask等等皆是基于AQS的。当然，我们自己也能利用AQS非常轻松容易地构造出符合我们自己需求的同步器。

#### 6.2.AQS原理分析

AQS 原理这部分参考了部分博客，在5.4节末尾放了链接。
...
![AQS原理图](../../../media/pictures/java/multithread/AQS原理图.png)
